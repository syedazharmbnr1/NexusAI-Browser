...
class AdvancedTabManager: ObservableObject {
    ...
    struct SmartTab: Identifiable {
        ...
        init(url: String = "https://www.google.com", purpose: TabPurpose = .general) {
            ...
            _ = WKUserContentController()   // reference not stored
...
class BrowserViewModel: ObservableObject {
    ...
    func executeAutomation(_ automation: SavedAutomation) async {
        ...
        do {
            let result = TaskAutomationResult(
                task: automation.name,
                steps: automation.actions.map { $0.description },
                automationScript: automation.script
            )
            
            try await smartAgent.executeAutomation(result, on: webView)
            showAgentPanel = true
        } catch {
...
struct BrowserView: View {
    ...
    var automationManagerPanel: some View {
        ...
        .padding()
        .frame(width: 450)
        .background(automationManagerBackground)
    ...
    var automationManagerBackground: some View {
        RoundedRectangle(cornerRadius: 12)
            .fill(Color.black.opacity(0.95))
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .stroke(LinearGradient(colors: [.green, .blue], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 1)
            )
    }
    ...
    var workflowPanel: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 12)
                        .stroke(LinearGradient(colors: [.yellow, .orange], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 1)
                )
        )
